...
/// Write a message to the unified log file (<exeDir>/IPMsgPro/ipmsgpro.log) AND std::cout
/// Only outputs when logging is enabled (SetLogEnabled(true)). Default: disabled.
void MsgLog(const std::string& msg);

/// Enable/disable logging. Default is disabled.
...
struct UserInfo {
    ...
    /// Unique key for identifying a user (userName@hostName/ipAddress)
    /// Includes IP to distinguish different machines with same username@hostname
    std::string Key() const {
        return userName + "@" + hostName + "/" + ipAddress;
    }
};
...
public:
    ...
    /// Scan an IP range by sending BR_ENTRY to each IP (startIp~endIp)
    /// Returns the number of IPs scanned
    int ScanIpRange(const std::string& startIp, const std::string& endIp);

    /// Scan multiple IP ranges
    void ScanIpRanges(const std::vector<std::pair<std::string, std::string>>& ranges);
