...
// ============================================================================
// Logger — unified via MsgLog (ipmsg/msgmng.h)
// ============================================================================
#define LOG_INFO(msg)  ipmsg::MsgLog(std::string("[INFO] ")  + msg)
#define LOG_WARN(msg)  ipmsg::MsgLog(std::string("[WARN] ")  + msg)
#define LOG_ERROR(msg) ipmsg::MsgLog(std::string("[ERROR] ") + msg)
#define LOG_DEBUG(msg) ipmsg::MsgLog(std::string("[DEBUG] ") + msg)
...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE, LPSTR lpCmdLine, int nCmdShow) {
    ...
    // Initialize logger (MsgLog auto-initializes on first call)
    std::string dataDir = GetAppDataDir(cliArgs.port);
    
    LOG_INFO("========================================");
    LOG_INFO("IPMsgPro starting...");
    LOG_INFO("Log file: " + dataDir + "\\ipmsgpro.log");
    ...
    app.OnSetup([&](tauricpp::App& app) {
        ...
        // NOTE: Do NOT broadcast entry here! The frontend will first load the
        // saved nickname from config and call config.set → UpdateLocalInfo,
        // which automatically re-broadcasts with the correct nickname.
        // The frontend's discoverUsers() call also triggers BroadcastEntry.
        // Broadcasting here would send the default login name ("User") before
        // the actual nickname is applied.
