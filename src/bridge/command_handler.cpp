...
#include "ipmsg/msgmng.h"  // for MsgLog
#include <ctime>
#include <random>
#include <sstream>
#include <fstream>
...
void CommandHandler::RegisterAllCommands() {
    ...
    bridge_->RegisterCommand("network.scan_ranges",
        [this](const nlohmann::json& args) { return HandleNetworkScanRanges(args); });
    ...
    msgMng_->SetMessageReceivedCallback([this](const MsgBuf& msg) {
        try {
            // Log every received message for debugging
            {
                ...
                MsgLog("[GUI-MSG] ====== BEGIN MESSAGE ======");
                MsgLog("[GUI-MSG] packetNo=" + std::to_string(msg.packetNo));
                MsgLog("[GUI-MSG] from=" + msg.sender.userName + "@" + msg.sender.hostName +
                       " (" + msg.sender.ipAddress + ":" + std::to_string(msg.sender.portNo) + ")");
                MsgLog("[GUI-MSG] nickName=" + msg.sender.nickName +
                       ", groupName=" + msg.sender.groupName);
                MsgLog("[GUI-MSG] command=" + std::string(cmdBuf) + " (" + modeStr + ")");
                std::string flags;
                ...
                MsgLog("[GUI-MSG] command_flags: " + flags);
                MsgLog("[GUI-MSG] body=\"" + msg.body + "\" (len=" + std::to_string(msg.body.size()) + ")");
                MsgLog("[GUI-MSG] extra=\"" + msg.extra + "\" (len=" + std::to_string(msg.extra.size()) + ")");
                std::stringstream extraHex;
                for (size_t i = 0; i < msg.extra.size() && i < 200; ++i) {
                    extraHex << std::hex << std::setfill('0') << std::setw(2) 
                             << (unsigned int)(unsigned char)msg.extra[i] << " ";
                }
                MsgLog("[GUI-MSG] extra_hex: " + extraHex.str());
                MsgLog("[GUI-MSG] ====== END MESSAGE =====");
            ...
            MsgLog("[GUI-MSG] Exception in message callback: " + std::string(e.what()));
            WriteDebugLog(std::string("[GUI-MSG] Exception: ") + e.what());
        } catch (...) {
            MsgLog("[GUI-MSG] Unknown exception in message callback");
...
nlohmann::json CommandHandler::HandleConfigSet(const nlohmann::json& args) {
    ...
    if (!nickname.empty() || !group.empty()) {
        ...
        MsgLog("[BACKEND-CONFIG] Updated: nickname=" + nickname + ", group=" + group);
    }
...
nlohmann::json CommandHandler::HandleMessageSend(const nlohmann::json& args) {
    ...
    MsgLog("[BACKEND-SEND] TEXT to=" + target->Key() + ", content=\"" + content + "\"");
...
nlohmann::json CommandHandler::HandleMessageSendImage(const nlohmann::json& args) {
    ...
    MsgLog("[BACKEND-SEND] IMAGE to=" + target->Key() + ", filePath=\"" + filePath + "\"");
...
nlohmann::json CommandHandler::HandleFileSend(const nlohmann::json& args) {
    ...
    MsgLog("[BACKEND-SEND] FILE to=" + target->Key() + ", filePath=\"" + filePath + "\"");
    ...
    // Normal mode: send UDP notification with file attachment info
    MsgLog("[BACKEND-SEND] Sending SENDMSG with FILEATTACHOPT to " + target->Key() +
           " (fileId=" + std::to_string(fileInfo->fileId) + ", fileSize=" + std::to_string(fileInfo->fileSize) + ")");
    MsgLog("[BACKEND-SEND] File attach info: " + fileAttachInfo);
    
    uint64_t sentPktNo = msgMng_->SendMessageWithFile(*target, "[File: " + fileInfo->fileName + "]", fileAttachInfo, IPMSG_SENDCHECKOPT);

    if (sentPktNo > 0) {
        MsgLog("[BACKEND-SEND] SENDMSG sent successfully, packetNo=" + std::to_string(sentPktNo));
        // Store the SENDMSG packetNo in FileInfo for matching GETFILEDATA requests
        {
            auto fi = fileTransfer_->GetFileInfo(transferId);
            if (fi) {
                ...
                fileTransfer_->RegisterFileInfo(transferId, *fi);
                MsgLog("[BACKEND-SEND] FileInfo updated: packetNo=" + std::to_string(sentPktNo) + ", fileId=" + std::to_string(fi->fileId));
            }
        }
        // Save to database
        MessageRecord record;
...
nlohmann::json CommandHandler::HandleFileRecv(const nlohmann::json& args) {
    ...
    MsgLog("[BACKEND-RECV] FILE from=" + target->Key() + ", fileName=\"" + fileName + "\", fileSize=" + std::to_string(fileSize) + ", savePath=\"" + savePath + "\"");
...
nlohmann::json CommandHandler::HandleNetworkScanRanges(const nlohmann::json& args) {
    // Parse ipRanges from frontend: [{ startIp: "10.3.120.1", endIp: "10.3.120.254" }, ...]
    std::vector<std::pair<std::string, std::string>> ranges;

    if (args.contains("ipRanges") && args["ipRanges"].is_array()) {
        for (const auto& range : args["ipRanges"]) {
            std::string startIp = range.value("startIp", "");
            std::string endIp = range.value("endIp", "");
    ...
    MsgLog("[BACKEND-SCAN-RANGES] Scanning " + std::to_string(ranges.size()) + " IP ranges...");

    // Run scan in a separate thread to avoid blocking the UI
    // The scan sends BR_ENTRY to each IP in the range
    std::thread([this, ranges]() {
        msgMng_->ScanIpRanges(ranges);
    }).detach();

    return {{"success", true}, {"ranges", ranges.size()}};
}
